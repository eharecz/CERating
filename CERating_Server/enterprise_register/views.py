...
def Enterprise_Register(request):
    ...
    print(req["email"])
    na = request.POST.get('businessName') #企业名称
    ty = request.POST.get('businessType1') #企业类型
    re = request.POST.get('businessType2') #企业从属关系：0总公司、1分公司、2其他
    em = request.POST.get('email') #相关负责人邮箱地址
    pa = request.POST.get('password') #密码的sha256加密密文
    ca = request.POST.get('securityCode') #注册邮箱验证码
